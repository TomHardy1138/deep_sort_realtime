...
class CUHK03(ImageDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        import os
        os.makedirs("cuhk03_test", exist_ok=True)
        folder_path = osp.join(self.root, "cuhk03_test")
        for img, pid, _ in gallery:
            #img = os.path.join(self.root, "cuhk03", img.split('/')[-2], img.split('/')[-1])
        ...
        os.makedirs("cuhk03_test", exist_ok=True)
        folder_path = osp.join(self.root, "cuhk03_test")
        for img, pid, _ in query:
            #img = os.path.join(self.root, "cuhk03", img.split(osp.sep)[-2], img.split(osp.sep)[-1])
            pid_folder = osp.join(folder_path, f"{pid}")
            os.makedirs(pid_folder, exist_ok=True)
            img_path = f"query_{img.split(osp.sep)[-1]}"
            img_path = osp.join(pid_folder, img_path)
            copyfile(img, img_path)
        os.makedirs("cuhk03_train", exist_ok=True)
        folder_path = osp.join(self.root, "cuhk03_train")
        for img, pid, _ in train:
            #img = os.path.join(self.root, "cuhk03", img.split(osp.sep)[-2], img.split(osp.sep)[-1])
            pid_folder = osp.join(folder_path, f"{pid}")
            os.makedirs(pid_folder, exist_ok=True)
            img_path = f"train_{img.split(osp.sep)[-1]}"
            img_path = osp.join(pid_folder, img_path)
            copyfile(img, img_path)
