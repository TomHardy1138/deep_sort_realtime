...
class BackgroundRemoving(object):
    """
    This augmentation is used to remove background from image
    Used heavy semantic segmentation model, so we picked p == 0.25
    """
    def __init__(self, prob=0.2):
        ckpt_path = str(pathlib.Path("ckpt", "ade20k-resnet50dilated-ppm_deepsup"))
        encoder_weights = pathlib.Path(ckpt_path, "encoder_epoch_20.pth")
        decoder_weights = pathlib.Path(ckpt_path, "decoder_epoch_20.pth")
        self.pil_to_tensor = Compose(
            [
                ToTensor(),
        ...
        self.colors = sio.loadmat("data/color150.mat")["colors"]
    ...
    def __call__(self, input_tuple, *args, **kwargs):
        ...
        path = img.filename
        temp = path.split(Path.sep)

        if "new_post" in temp:
            temp[-3] = f"{temp[-3]}_aug"
            img = Image.open(Path.sep.join(temp))
            return img, mask


        # img_data = self.pil_to_tensor(img)
        ...
        # # If person class isn't confident
        # if predicted_classes[0] != self.PERSON_IDX:
        #     return input_tuple

        # pred[pred != self.PERSON_IDX] = -1
        # segm_mask = colorEncode(pred, self.colors).astype(np.uint8)
...
def build_transforms(height, width, transforms=None, norm_mean=(0.485, 0.456, 0.406),
    ...
    if transforms.segmentation.enable:
        print("+ segmentation for background removal")
        transform_tr += [BackgroundRemoving()]
    ...
    if transforms.random_blur.enable:
        print('+ random_blur')
        transform_tr += [
            GaussianBlur(p=transforms.random_blur.p,
