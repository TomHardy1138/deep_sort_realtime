...
class SemiHardTripletLoss(nn.Module):
    def __init__(self, margin=0.35):
        super().__init__()
        self.margin = margin
        self.circle = CircleLoss(0.25, 32)
    ...
    def forward(self, features, labels):
        ...
        # fts = features.div(fnorm.expand_as(features))
        # circle_loss = self.circle(*convert_label_to_similarity(fts, labels)) / 32
        ###
