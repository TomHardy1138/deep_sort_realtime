...
class LCTGate(nn.Module):
    ...
    def forward(self, x):
        ...
        if not torch.onnx.is_in_onnx_export():
            ...
            size = x.size()
            x = x.view(1, self.gn.num_groups, -1)
            mean = x.mean(dim=2, keepdim=True)
            data_mean_squared = mean ** 2
            data_squared = x ** 2
            data_squared_mean = torch.mean(data_squared, dim=2, keepdim=True)
            std = torch.sqrt(data_squared_mean - data_mean_squared)

            x = (x - mean) / (std + self.gn.eps)
            x = x.view(size)
            x = x * self.gn.weight.reshape(1, size[1], 1, 1) + self.gn.bias.reshape(1, size[1], 1, 1)
...
class OSNetFPN(OSNet):
    ...
    def __init__(self, num_classes, blocks, layers, channels,
        ...
        if self.loss not in ['am_softmax', ]:
            ...
            self.classifier = AngleSimpleLinearV2(self.feature_dim, num_classes)
    ...
    def featuremaps(self, x):
        ...
        if self.use_IN_first:
            if not torch.onnx.is_in_onnx_export():
                ...
                s = self.in_first.weight
                bias = self.in_first.bias
                eps = self.in_first.eps
                dims_x = len(x.size())
                axis = tuple(range(2, dims_x))
                mean = torch.mean(x, axis=axis, keepdims=True)

                # variance computing
                data_mean_squared = mean ** 2
                data_squared = x ** 2
                data_squared_mean = torch.mean(data_squared, axis=axis, keepdims=True)
                var = torch.sqrt(data_squared_mean - data_mean_squared) ** 2

                dim_ones = (1,) * (dims_x - 2)
                s = s.reshape(-1, *dim_ones)
                bias = bias.reshape(-1, *dim_ones)
                x = s * (x - mean) / torch.sqrt(var + eps) + bias
    ...
    def forward(self, x, return_featuremaps=False, get_embeddings=False):
        if torch.onnx.is_in_onnx_export():
            mean = torch.Tensor([0.485*255, 0.456*255, 0.406*255]).view(1, -1, 1, 1)
            std = torch.Tensor([0.229*255, 0.224*255, 0.225*255]).view(1, -1, 1, 1)
