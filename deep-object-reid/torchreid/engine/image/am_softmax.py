...
from .centroid import CentroidTripletLoss
...
class ImageAMSoftmaxEngine(Engine):
    ...
    def __init__(self, datamanager, model, optimizer,
                 reg_cfg, metric_cfg, batch_transform_cfg,
        ...
        self.additional_optimizer = None
        self.loss_name = ""
        ...
        self.ml_losses = list()
        self.additional_losses = nn.ModuleList()
        ...
        for trg_id, trg_num_classes in enumerate(self.num_classes):
            ...
            if softmax_type == 'stock':
                ...
                self.main_losses.append(ArcfaceLoss(pr_product=pr_product))

                if metric_cfg.additional_losses.name == "cross_batch":
                    self.additional_losses.append(
                        losses.CrossBatchMemory(
                            losses.MultiSimilarityLoss(),
                            self.model.module.feature_dim,
                            memory_size=2048, #8192,
                            miner=None
                        )
                    )
                elif metric_cfg.additional_losses.name == "circle":
                    print("Using Circle Loss...")
                    self.additional_losses.append(losses.CircleLoss(m=0.25, gamma=128))
                elif metric_cfg.additional_losses.name == "contrastive":
                    print("Using Contrasrive Loss...")
                    self.additional_losses.append(losses.ContrastiveLoss())
                elif metric_cfg.additional_losses.name == "nt_xent":
                    print("Using NTXentLoss...")
                    self.additional_losses.append(losses.NTXentLoss())
                elif metric_cfg.additional_losses.name == "intra_pair":
                    print("Using IntraPair Loss...")
                    self.additional_losses.append(
                ...
                elif metric_cfg.additional_losses.name == "sup_con_loss":
                    print("Using SupConLoss...")
                    self.additional_losses.append(losses.SupConLoss())
                elif metric_cfg.additional_losses.name == "structure":
                    print("Using LiftedStructureLoss...")
                    self.additional_losses.append(losses.LiftedStructureLoss())
                elif metric_cfg.additional_losses.name == "general":
                    print("Using GeneralizedLiftedStructureLoss...")
                    self.additional_losses.append(losses.GeneralizedLiftedStructureLoss())
                elif metric_cfg.additional_losses.name == "margin":
                    print("Using MarginLoss...")
                    self.additional_losses.append(losses.MarginLoss(num_classes=self.num_classes))
                elif metric_cfg.additional_losses.name == "multi_sim":
                    print("Using MultiSimilarityLoss...")
                    self.additional_losses.append(losses.MultiSimilarityLoss())  
                elif metric_cfg.additional_losses.name == "nca":
                    print("Using NCALoss...")
                    self.additional_losses.append(losses.NCALoss())
                elif metric_cfg.additional_losses.name == "triplet_margin":
                    print("Using TripletMarginLoss...")
                    self.additional_losses.append(losses.TripletMarginLoss())
                elif metric_cfg.additional_losses.name == "angular":
                    print("Using AngularLoss...")
                    self.additional_losses.append(losses.AngularLoss())
                elif metric_cfg.additional_losses.name == "noise_ratio":
                    print("Using SignalToNoiseRatioContrastiveLoss...")
                    self.additional_losses.append(losses.SignalToNoiseRatioContrastiveLoss())
                elif metric_cfg.additional_losses.name == "mix":
                    print("Using MultipleLosses...")
                    self.additional_losses.append(
                        losses.MultipleLosses(
                            [
                                losses.LiftedStructureLoss(), # good R@1 for zero and zero point five threshold
                                losses.SignalToNoiseRatioContrastiveLoss(), # good R@1 for zero threshold
                                losses.NTXentLoss() # third best result for all threshold
                            ],
                            weights=[2.0, 1.0, 0.5]
                        )
                    )
                elif metric_cfg.additional_losses.name == "mix_2":
                    print("Using MultipleLosses_2...")
                    self.additional_losses.append(
                        losses.MultipleLosses(
                            [
                                losses.LiftedStructureLoss(), # good R@1 for 0 and 0.5 threshold
                                losses.SignalToNoiseRatioContrastiveLoss(), # good R@1 0 threshold
                                losses.NTXentLoss(), # third best result for all thresholds
                                losses.ContrastiveLoss() # tied best result with NTXent, better for 0.5
                            ],
                            weights=[2.0, 2.0, 0.5, 0.5]
                        )
                    )
                elif metric_cfg.additional_losses.name == "mix_3":
                    print("Using MultipleLosses_3...")
                    self.additional_losses.append(
                        losses.MultipleLosses(
                            [
                                losses.LiftedStructureLoss(), # good R@1 for 0 and 0.5 threshold
                ...
                elif metric_cfg.additional_losses.name == "mix_4":
                    print("Using MultipleLosses_4...")
                    self.additional_losses.append(
                        losses.MultipleLosses(
                            [
                                losses.LiftedStructureLoss(), # good R@1 for 0 and 0.5 threshold
                            ...
                            weights=[2.0, 1.0, 0.5, 0.5, 0.25]
                        )
                    )
                elif metric_cfg.additional_losses.name == "mix_5":
                    print("Using ML + CTL...")
                    self.additional_losses.append(
                        losses.MultipleLosses(
                            [
                                ...
                                losses.NTXentLoss(), # third best result for all thresholds
                                losses.ContrastiveLoss(), # tied best result with NTXent, better for 0.5
                                CentroidTripletLoss(margin=0.3)
                            ],
                            weights=[2.0, 1.0, 0.5, 0.5, 1.0]
                        )
                        
                    )
                elif metric_cfg.additional_losses.name == "mix_6":
                    print("Using ML + CTL...")
                    self.additional_losses.append(
                        losses.MultipleLosses(
                            [
                            ...
                            weights=[2.0, 1.0, 0.5, 0.5, 0.5]
                        )
                        
                    )
                elif metric_cfg.additional_losses.name == "head_pal":
                    print("Using ML + ProxyAnchorLoss...")
                    pal_loss = losses.ProxyAnchorLoss(
                        num_classes=num_classes[-1],
                        embedding_size=256
                    ).cuda()
                    self.additional_optimizer = torch.optim.SGD(pal_loss.parameters(), lr=0.01)
                    self.additional_losses.append(
                        losses.MultipleLosses(
                            [
                                losses.LiftedStructureLoss(), # good R@1 for 0 and 0.5 threshold
                ...
                elif metric_cfg.additional_losses.name == "head_pal_2":
                    print("Using ML + ProxyAnchorLoss...")
                    pal_loss = losses.ProxyAnchorLoss(
                        num_classes=num_classes[-1],
                        embedding_size=256
                    ...
                    self.additional_losses.append(
                        losses.MultipleLosses(
                            ...
                            weights=[2.0, 1.0, 0.5, 0.5, 1.0, 0.25]
                        )
                    )
                elif metric_cfg.additional_losses.name == "head_pnca":
                    print("Using ML + ProxyNCALoss...")
                    pnca_loss = losses.ProxyNCALoss(
                        num_classes=num_classes[-1],
                        embedding_size=256
                    ).cuda()
                    self.additional_optimizer = torch.optim.SGD(pnca_loss.parameters(), lr=0.01)
                    self.additional_losses.append(
                        losses.MultipleLosses(
                            [
                                losses.LiftedStructureLoss(), # good R@1 for 0 and 0.5 threshold
                ...
                elif metric_cfg.additional_losses.name == "head_stl":
                    print("Using ML + SoftTripleLoss...")
                    stl_loss = losses.SoftTripleLoss(
                        num_classes=num_classes[-1],
                        embedding_size=256
                    ).cuda()
                    self.additional_optimizer = torch.optim.SGD(stl_loss.parameters(), lr=0.01)
                    self.additional_losses.append(
                        losses.MultipleLosses(
                            [
                                losses.LiftedStructureLoss(), # good R@1 for 0 and 0.5 threshold
                ...
                elif metric_cfg.additional_losses.name == "head_lmsl":
                    print("Using ML + LargeMarginSoftmaxLoss...")
                    lmsl_loss = losses.LargeMarginSoftmaxLoss(
                        num_classes=num_classes[-1],
                        embedding_size=256
                    ).cuda()
                    self.additional_optimizer = torch.optim.SGD(lmsl_loss.parameters(), lr=0.01)
                    self.additional_losses.append(
                        losses.MultipleLosses(
                            [
                                losses.LiftedStructureLoss(), # good R@1 for 0 and 0.5 threshold
                ...
                elif metric_cfg.additional_losses.name == "head_nsl":
                    print("Using ML + NormalizedSoftmaxLoss...")
                    nsl_loss = losses.NormalizedSoftmaxLoss(
                        num_classes=num_classes[-1],
                        embedding_size=256
                    ).cuda()
                    self.additional_optimizer = torch.optim.SGD(nsl_loss.parameters(), lr=0.01)
                    self.additional_losses.append(
                        losses.MultipleLosses(
                            [
                                losses.LiftedStructureLoss(), # good R@1 for 0 and 0.5 threshold
                    ...
                    raise ValueError('Another loss is not supported...')

                self.loss_name = metric_cfg.additional_losses.name

                # self.main_losses.append(AdaCos(num_classes=num_classes[0]))
                ...
                print(self.main_losses)
                print(self.additional_losses)
    ...
    def forward_backward(self, data):
        ...
        if self.additional_optimizer is not None:
        ...
        for trg_id in range(self.num_targets):
            ...
            if self.enable_metric_losses:
                ...
                metric_loss = self.additional_losses[trg_id](embd, trg_obj_ids) # * 2
                loss_summary['add_{}'.format(trg_id)] = metric_loss.item()
                trg_loss += metric_loss

                del embd

            # """Don't work on torch > 1.4.0"""
            # Because loss backward
            # if self.enable_metric_losses:
