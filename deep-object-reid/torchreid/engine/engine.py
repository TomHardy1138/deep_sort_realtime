    ...
    def _evaluate(
        ...
        def _feature_extraction(data_loader):
            ...
            for batch_idx, data in enumerate(data_loader):
                ...
                # with open(f"{batch_idx}_orig.txt", "w") as file:
                #     for f in imgs.reshape(-1):
                #         file.write(f"{f}\n")
                # file.close()
                # with open(f"{batch_idx}_features.txt", "w") as file:
                #     for f in features.reshape(-1):
                #         file.write(f"{f}\n")
                # file.close()
        ...
        np.save(f"{dataset_name}_fts.npy", torch.cat((gf, qf), 0).numpy())
        np.save(f"{dataset_name}_pids.npy", np.concatenate((g_pids, q_pids), 0))
        ...
        # distmat = np.where(distmat >= 0.5, distmat, 0)
