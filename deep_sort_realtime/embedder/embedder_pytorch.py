...
class Karen_Embedder(object):
    ...
    def __init__(
        self, model_wts_path=None, half=True, max_batch_size=16, bgr=False, gpu=True
    ):
        assert os.path.exists(
            model_wts_path
        ), f"OSNet model path {model_wts_path} does not exists!"
        
        import torchreid
        
        self.model = torchreid.models.osnet_fpn.fpn_osnet_x1_0(
            num_classes=80793,
            feature_dim=256,
            IN_first=True,
            pooling_type='conv',
        ...
        torchreid.utils.load_pretrained_weights(self.model, model_wts_path)
        ...
        logger.info("Karen Embedder for Deep Sort initialised")
        logger.info(f"- gpu enabled: {self.gpu}")
        logger.info(f"- half precision: {self.half}")
        logger.info(f"- max batch size: {self.max_batch_size}")
        logger.info(f"- expects BGR: {self.bgr}")
    ...
    def preprocess(self, np_image):
        ...
        input_image = cv2.resize(np_image_rgb, (256, 128))
        trans = transforms.Compose(
            [
                transforms.ToTensor(),
                transforms.Normalize(
        ...
        input_image = trans(input_image)
        input_image = input_image.view(1, 3, 256, 128)
